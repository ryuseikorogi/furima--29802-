class PayForm
  ...
  with_options presence: true do
    validates  :city
    validates  :address
    validates  :phone_number,length: {maximum:11}#文字数の制限のvalidation追加 購入画面でerrorメッセージの確認 もし出なければviewにエラーメッセージの記述が出る
    validates  :token
    
  end
  validates :shipping_prefecture_id, numericality: { other_than: 0, message: "can't be blank" }

  validates :zip_code, format: {with: /\A[0-9]{3}-[0-9]{4}\z/, message: "can't be blank"}
# ここにバリデーションの処理を書く
